...
To update the database use this command from the Web project folder (replace `CleanArchitectureTest` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../CleanArchitectureTest.Infrastructure/CleanArchitectureTest.Infrastructure.csproj -s CleanArchitectureTest.Web.csproj
```
